...
object BarnacleSubMenu {
    ...
    fun buildGroups(category: ConfigCategory.Builder) {
        category.group(OptionGroup.createBuilder()
            .name(Component.translatable("entity.squ_abyssal_bloom.barnacle").copy().withStyle(ChatFormatting.DARK_AQUA))
            ...
            .option(serverInt(ModServerConfig.BARNACLE_SPAWN_MAX_Y) { Component.literal("Y=$it") })
            .option(serverDouble(ModServerConfig.BARNACLE_DETECTION_RANGE, step = 1.0, format = ModUtilities.blocksFormatDouble()))
            ...
            .option(serverInt(ModServerConfig.BARNACLE_REGEN_COOLDOWN, step = 20, format = ModUtilities.ticksFormat()))
            .option(serverDouble(ModServerConfig.BARNACLE_DIRECT_CORRIDOR_DISTANCE, step = 0.5, format = ModUtilities.blocksFormatDouble()))
            .option(serverDouble(ModServerConfig.BARNACLE_CAPTURE_DISTANCE, step = 0.25, format = ModUtilities.blocksFormatDouble()))
            .option(serverDouble(ModServerConfig.BARNACLE_HOLD_DISTANCE, step = 0.25, format = ModUtilities.blocksFormatDouble()))
            .build())
    }
}
